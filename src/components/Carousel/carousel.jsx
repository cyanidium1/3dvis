...
const Carousel = ({ portfolioPosts }) => {
    ...
    const handlePrevClick = () => {
        setActiveIndex((prevIndex) => (prevIndex === 0 ? portfolioPosts.length - 1 : prevIndex - 1));
        sliderRef.current.prepend(sliderRef.current.lastChild);
    };

    const handleNextClick = () => {
        setActiveIndex((prevIndex) => (prevIndex === portfolioPosts.length - 1 ? 0 : prevIndex + 1));
    ...
    return (
        <div className="animBox h-screen overflow-hidden">
            <div className="slider" ref={sliderRef}>
                {portfolioPosts && portfolioPosts.map((el, index) => (
                    <div className={`item ${index === activeIndex ? "active" : ""}`} key={index}>
                        <img className="carousel-photo" src={portfolioPosts[index].coverImage.url} loading="eager" alt="" />
                        <div className="content w-[40vw] md:w-96 left-2 md:left-8  bg-dark-brown bg-opacity-50 p-2 md:p-6">
                            ...
                            <p className="font-extralight text-xs md:text-lg uppercase tracking-widest flex items-center">{truncateDescription(el.description)}</p>
                            <Link href={`/portfolio/${el.slug}`}>
                                <p className="btn justify-center bg-brown text-white text-center uppercase px-6 py-4 text-xs w-full md:w-fit">Read more</p>
                            </Link>
                        </div>
                    </div>
                ))}
